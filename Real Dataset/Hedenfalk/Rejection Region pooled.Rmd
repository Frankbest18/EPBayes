...
sd1 = sqrt(var(X1_1))
sd2 = sqrt(var(X2_1))
se_pool = sqrt((1/K1 + 1/K2) * ((K1-1) * sd1^2 + (K2-1) * sd2^2)/(K1+K2-2))
md = m1 - m2
t = md / se_pool
```
...
for (i in 1:nrow(Hedenfalk)) {
  ...
  s1_i = var(X1[i, ])
  s2_i = var(X2[i, ])
  t_pool_i = (m1_i - m2_i) / sqrt((1/K1 + 1/K2) * ((K1-1) * s1_i + (K2-1) * s2_i)/(K1+K2-2))
  t_pool_list = c(t_pool_list, t_pool_i)
}
max(t_pool_list)

t_pool_list = t_pool_list[log(tau_list) >= -5]
```
...
bfresult
pool_pvalue_BF(t, sd1^2/sd2^2, K1, K2)
```
...
for (tau in tau_grid) {
  t_BF_005_i = pool_pvalue_BF_solver_positive(tau, K1, K2, 0.05)
...
pool_pvalue_1DNPMLE(t, K1, K2, grid_1D, mass, sd1^2/sd2^2)
pool_pvalue_1DNPMLE_solver_positive(K1, K2, grid_1D, mass, sd1^2/sd2^2, 0.05)
```
...
for (tau in tau_grid) {
  t_1DNPMLE_005_i = pool_pvalue_1DNPMLE_solver_positive(K1, K2, grid_1D, mass, tau, 0.05)
  t_1DNPMLE_bh_i = pool_pvalue_1DNPMLE_solver_positive(K1, K2, grid_1D, mass, tau, cutoff_1DNPMLE)
...
pool_pvalue_Welch(t, sd1^2/sd2^2, K1, K2)
pool_pvalue_Welch_solver_positive(sd1^2/sd2^2, K1, K2, 0.05)
t
```
...
for (tau in tau_grid) {
  t_Welch_005_i = pool_pvalue_Welch_solver_positive(tau, K1, K2, 0.05)
  t_Welch_bh_i = pool_pvalue_Welch_solver_positive(tau, K1, K2, cutoff_Welch)
...
pool_pvalue_t(t, sd1^2/sd2^2, K1, K2)
pool_pvalue_t_solver_positive(sd1^2/sd2^2, K1, K2, 0.05)
t
```
...
for (tau in tau_grid) {
  t_t_005_i = pool_pvalue_t_solver_positive(tau, K1, K2, 0.05)
  t_t_bh_i = pool_pvalue_t_solver_positive(tau, K1, K2, cutoff_pooled_t)
